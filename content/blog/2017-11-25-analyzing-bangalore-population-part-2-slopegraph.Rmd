...
This is part 2 of of the population analysis of Bangalore using R. In [part 1 we explored how to make animated choropleth on timeseries populataion data](https://www.shyamvalsan.com/blog/analyzing-bangalore-population-animated-choropleth-timeseries/). While the previous visualization we did was geographical in nature, this time we’re going to visualize Bangalore population data using a slopegraph. Slopegraphs help paint a before and after story by providing insights into ranking, magnitude and changes over time. [Part 3](https://shyamvalsan.com/blog/analyzing-bangalore-population-smoothed-line-graph/ ) and [Part 4](https://shyamvalsan.com/blog/analyzing-bangalore-population-bubblecharts/) are also now published.
...
for(val in 1:(length(columns)-1)){
  ...
  if(val == length(columns)-1){
    ...
    text(unique(x_tmp[[2]]),y_plot[2],heading)
  }	
  
  heading<-substitute(bold(x),list(x=names(temp_plot)[1]))  
  text(unique(x_tmp[[1]]),y_plot[2],heading)
